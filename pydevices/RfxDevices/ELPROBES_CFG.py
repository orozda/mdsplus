...
class ELPROBES_CFG(Device):
    ...
    for i in range(0,99):
        ...
        parts.append({'path':'.PROBE_%02d:FRONT_END_CH'%(i+1), 'type':'numeric'})
        parts.append({'path':'.PROBE_%02d:ADC_SLOW'%(i+1), 'type':'text'})
        parts.append({'path':'.PROBE_%02d:ADC_SLOW_CON'%(i+1), 'type':'text'})
        parts.append({'path':'.PROBE_%02d:ADC_FAST'%(i+1), 'type':'text'})
        parts.append({'path':'.PROBE_%02d:CALIB_RV'%(i+1), 'type':'numeric'})
        parts.append({'path':'.PROBE_%02d:CALIB_RI_H'%(i+1), 'type':'numeric'})
        parts.append({'path':'.PROBE_%02d:CALIB_RI_L'%(i+1), 'type':'numeric'})
        parts.append({'path':'.PROBE_%02d:NOTE'%(i+1), 'type':'text'})
        parts.append({'path':'.PROBE_%02d:BIAS_SOURCE'%(i+1), 'type':'text'})
    ...
    def write_probe_tag(self):
        ...
        try :
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot remove probes tag, experiment must be open in Edit Mode ' + str(e) )
