...
class LASER_QSMART(Device):
    ...
    flqsModeDict = {'FL-INT QS-INT':'II' , 'FL-INT QS-EXT':'IE', 'FL-EXT QS-INT':'EI', 'FL-EXT QS-EXT':'EE'}
    STATUS_OK   = 0
    STATUS_ERR  = -1
    STATUS_TMO  = -2
    ...
    def writeParam(self, sock, cmd, node, conv, msg):
        ...
        if node != None : 
            try:
                ...
                Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot resolve %s %s'%(msg, str(e)) )
        ...
        try:
            ...
            if status == -1:
                Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing %s %s'%(msg, out) )
                raise mdsExceptions.DevCOMM_ERROR
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing %s %s'%(msg, str(e)) )
            raise mdsExceptions.DevCOMM_ERROR
...
#Support class for Laser shots
    class AsynchStart(Thread):
      ...
      def configure(self, device, ip_address, port, stopArm):
          ...
          if self.sock == -1:
              Data.execute('DevLogErr($1,$2)', self.device.getNid(), 'Cannot open device '+ msg )
              raise mdsExceptions.DevCOMM_ERROR
      ...
      def run(self):
          ...
          try:
              ...
              status, currState = self.device.stateMonitor(self.sock, self.device.STATE2, 3)
              if  status == self.device.STATUS_OK :
                  self.device.writeParam(self.sock, 'RUN'        , None             , None        , 'RUN Activates the Flash lamps')
              else:
              ...
              if  status == self.device.STATUS_OK :
                  ...
                  status, currState = self.device.stateMonitor(self.sock, self.device.STATE5, 10)
                  ...
                  if  status == self.device.STATUS_OK :
                      self.device.writeParam(self.sock, 'QSW 1'        , None             , None        , 'QSW Starts the Q-Switch laser emission')
                  else:
              ...
              if  status == self.device.STATUS_OK :
                  self.device.writeParam(self.sock, 'QSW 0'        , None             , None        , 'QSW Stops the Q-Switch laser emission')
              else:
              ...
              if  status == self.device.STATUS_OK :
                  ...
                  status, currState = self.device.stateMonitor(self.sock, self.device.STATE5, 3)
                  if  status == self.device.STATUS_OK :
                      self.device.writeParam(self.sock, 'STOP'         , None             , None        , 'STOP Stops the flash lamps')
                  else:
                      print('WARNING: Laser not in Flashing state')
...
####################################################################
    ...
    def init(self):
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot resolve IP address '+ str(e) )
            raise mdsExceptions.DevBAD_PARAMETER

        try:
            port = self.port.data()
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot resolve IP port '+ str(e) )
            raise mdsExceptions.DevBAD_PARAMETER

        sock, msg = self.laserOpen(ip_address, port)
        if sock == -1:
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot open device '+ msg )
            raise mdsExceptions.DevCOMM_ERROR

        status, currState = self.stateMonitor(sock, self.STATE2, 15)
        if  status != self.STATUS_OK :
            self.laserClose(sock)
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Laser not reaches the ready to run state (2), current state %s'%(self.stateToMsg[currState]) )
            raise mdsExceptions.DevIO_STUCK


        try:
            self.writeParam(sock, 'CAPVSET'    , self.fl_voltage  , None              , 'Flashlamp voltage')
            self.writeParam(sock, 'LPW'        , self.fl_pwidth   , None              , 'Flashlamp pulse width')
            self.writeParam(sock, 'TRIG'       , self.flqs_mode   , self.flqsModeDict , 'Flashlamp Qswitch mode')
            self.writeParam(sock, 'QSPAR1'     , self.qs_par1     , None              , 'QSPAR1 number of cycles for Burst/Scan mode')
            self.writeParam(sock, 'QSPAR2'     , self.qs_par2     , None              , 'QSPAR2 number of passive pulses per cycle for Burst/Scan mode')
            self.writeParam(sock, 'QSPAR3'     , self.qs_par3     , None              , 'QSPAR3 number of active Q-Switch pulses per cycle for Burst/Scan mode')
            self.writeParam(sock, 'QDLY'       , self.delay_flqs  , None              , 'DELAY_FLQS delay between the end of the flashlamp pulse and the beginning of the Q-S pulse')
            self.writeParam(sock, 'QDLYO'      , self.delay_qsqso , None              , 'DELAY_QSQSO delay between the Q-Switch pulse and the Q-Switch Synch. Out')
    ...
    def arm(self):
        ...
        if self.restoreWorker() == 1 :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Laser is already armed' )
            raise mdsExceptions.DevCOMM_ERROR
    ...
    def stop(self):
        ...
        if self.restoreWorker() == -1 :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Laser not in arm state' )
            raise mdsExceptions.DevCOMM_ERROR
    ...
    def reset(self):
        ...
        if self.restoreWorker() == 1 :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Laser is armed. Reset operation not allowed' )
            raise mdsExceptions.DevCOMM_ERROR
        ...
        try:
            self.writeParam(sock, 'RESET'    , None  , None              , 'Laser reset ==> switch ON/OFF')
